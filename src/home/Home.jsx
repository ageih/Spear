...
function Home({ onUserLevelChange }) {
  ...
  // 🔥 CRITICAL FIX: Redirect level 2+ users away from dashboard on login
  useEffect(() => {
    if (currentUser && currentView === "dashboard") {
      // Check if user is level 2 or higher (not level 1)
      if (currentUser.cs_user_level >= 2) {
        console.log(
          `🚫 Level ${currentUser.cs_user_level} user detected, redirecting from dashboard`
        );

        // Choose appropriate default view based on user level
        let defaultView = "letter-tracker";

        if (currentUser.is_staff) {
          defaultView = "staff-tasks";
        } else {
          // Customize default views by level
          switch (currentUser.cs_user_level) {
            case 2: // Department Head
              defaultView = "letter-tracker";
              break;
            case 3: // Team Lead
              defaultView = "policy-tracker";
              break;
            case 4: // Senior Staff
              defaultView = "meeting-modes";
              break;
            case 5: // Basic Staff
              defaultView = currentUser.is_staff
                ? "staff-tasks"
                : "letter-tracker";
              break;
            default:
        ...
        console.log(`🔀 Redirecting to: ${defaultView}`);
  ...
  // 🔥 UPDATED: Render different navigation based on user level
  const renderNavigation = () => {
    if (!userPermissions) {
      console.warn("⚠️ Cannot render navigation: No user permissions");
      return null;
    ...
    // 🔥 CRITICAL: Check if user is level 1 (only level 1 sees dashboard)
    const isLevel1User = currentUser?.cs_user_level === 1;
      ...
      <nav className="action-navigation">
        <div className="nav-section">
          ...
          {isLevel1User ? (
            ...
            <button
              onClick={() => handleViewChange(getDefaultViewForLevel())}
              className={`nav-button ${
                currentView === getDefaultViewForLevel()
                  ? "nav-button-active"
                  : ""
              }`}
            >
  ...
  // Helper function to get default view for non-level-1 users
  const getDefaultViewForLevel = () => {
    if (!currentUser) return "letter-tracker";
    ...
    switch (currentUser.cs_user_level) {
      case 2: // Department Head
        return "letter-tracker";
      case 3: // Team Lead
        return "policy-tracker";
      case 4: // Senior Staff
        return "meeting-modes";
      case 5: // Basic Staff
        return currentUser.is_staff ? "staff-tasks" : "letter-tracker";
      default:
        return "letter-tracker";
    }
  };

  // Helper function to get default view label
  const getDefaultViewLabel = () => {
    if (!currentUser) return "Task Center";
  ...
  // 🔥 UPDATED: Render different content based on current view and user level
  const renderMainContent = () => {
    if (!currentUser || !userPermissions) {
      console.warn(
        "⚠️ Cannot render main content: Missing user or permissions"
    ...
    // 🔥 CRITICAL: Block dashboard access for non-level-1 users
    if (currentView === "dashboard" && currentUser.cs_user_level !== 1) {
      console.log(
        `🚫 Level ${currentUser.cs_user_level} user trying to access dashboard, showing access denied`
      );

      const defaultView = getDefaultViewForLevel();
          ...
          <div className="page-header">
            {/* <div className="access-denied-header">
              ...
              <p className="access-denied-text">
                Dashboard access is only available for Level 1 users.
                Your current access level ({currentUser.cs_user_level}) does not permit viewing the dashboard.
              </p>
              <p className="access-redirect-text">
                You have been redirected to the {getDefaultViewLabel()}.
              </p>
              <button
                className="primary-button redirect-button"
                onClick={() => handleViewChange(defaultView)}
              >
                Go to {getDefaultViewLabel()}
              </button>
            </div> */}
          </div>
          {renderContentForView(defaultView)}
        </div>
      );
    }

    return renderContentForView(currentView);
  };

  // Helper function to render content for different views
  const renderContentForView = (view) => {
    ...
    switch (view) {
      ...
      default:
        console.warn(`⚠️ Unknown view: ${view}, falling back to default view`);
        return renderContentForView(getDefaultViewForLevel());
    }
  };
          ...
          <div className="user-info">
            ...
            <div className="user-details">
              <div className="user-name">
                {currentUser.cs_user_name},Nagaland.
              </div>
              {/* <div className="user-level">Level {currentUser.cs_user_level}</div> */}
            </div>
          </div>
          <button onClick={handleLogout} className="logout-button">
            <svg
